from __future__ import annotations

from collections.abc import Mapping
from typing import Any, TypeVar, cast

from attrs import define as _attrs_define
from attrs import field as _attrs_field

from ..models.export_xer_project_file_type_type import ExportXERProjectFileTypeType
from ..types import UNSET, Unset

T = TypeVar("T", bound="ExportXERProject")


@_attrs_define
class ExportXERProject:
    """ExportXERProject Entity

    Attributes:
        file_type (ExportXERProjectFileTypeType | Unset): FileTypeType Entity
        project_object_id (list[int] | Unset):
    """

    file_type: ExportXERProjectFileTypeType | Unset = UNSET
    project_object_id: list[int] | Unset = UNSET
    additional_properties: dict[str, Any] = _attrs_field(init=False, factory=dict)

    def to_dict(self) -> dict[str, Any]:
        file_type: str | Unset = UNSET
        if not isinstance(self.file_type, Unset):
            file_type = self.file_type.value

        project_object_id: list[int] | Unset = UNSET
        if not isinstance(self.project_object_id, Unset):
            project_object_id = self.project_object_id

        field_dict: dict[str, Any] = {}
        field_dict.update(self.additional_properties)
        field_dict.update({})
        if file_type is not UNSET:
            field_dict["FileType"] = file_type
        if project_object_id is not UNSET:
            field_dict["ProjectObjectId"] = project_object_id

        return field_dict

    @classmethod
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        d = dict(src_dict)
        _file_type = d.pop("FileType", UNSET)
        file_type: ExportXERProjectFileTypeType | Unset
        if isinstance(_file_type, Unset):
            file_type = UNSET
        else:
            file_type = ExportXERProjectFileTypeType(_file_type)

        project_object_id = cast(list[int], d.pop("ProjectObjectId", UNSET))

        export_xer_project = cls(
            file_type=file_type,
            project_object_id=project_object_id,
        )

        export_xer_project.additional_properties = d
        return export_xer_project

    @property
    def additional_keys(self) -> list[str]:
        return list(self.additional_properties.keys())

    def __getitem__(self, key: str) -> Any:
        return self.additional_properties[key]

    def __setitem__(self, key: str, value: Any) -> None:
        self.additional_properties[key] = value

    def __delitem__(self, key: str) -> None:
        del self.additional_properties[key]

    def __contains__(self, key: str) -> bool:
        return key in self.additional_properties
